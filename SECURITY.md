...
|---|---|---|---|
| `contracts/LumenInsurance.py:1-2` | Info | Header is `# v0.2.16` (version comment) then the `Depends` hash, vs. the canonical docs example showing only the `Depends` line. | **Verified non-issue, not changed.** This exact header has produced `resultName: AGREE`/`FINISHED` on every one of ~9 live Bradbury deploys this session (most recently `0x85D3890C709aE8352A41Bc51DbEEB8b8aBBe728f`), including full 5-validator consensus. Confirmed 0 CRLF (`grep -c $'\r'` → 0). Changing a proven-working pattern to match a template with no evidence the extra line breaks anything would be pure risk for zero benefit. |
...
| `tests/direct/*` | — | Weather's `check_weather_trigger` has never been exercised against Bradbury live (only flight has, 3x). | Not a code defect — the deployer test account (`0xC6E6d3b2acCaECeCeB40Ad4bD3dF123DDCB4e537`) ran down to ~0.078 GEN after repeated live flight tests + redeploys this session, below the cost of even a 1 GEN test policy. Needs a funded wallet to complete; architecturally lower-risk than flight's finding since Open-Meteo is a pure JSON API with no rendering dependency, but this is asserted, not yet live-proven. |
