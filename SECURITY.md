...
5. **External data source availability is not guaranteed.** FlightAware and Open-Meteo are third-party services Lumen doesn't control; if either is unreachable or changes its page/response format, `_extract_claim_facts` degrades to `record_matches_*: False` (fail-closed — a claim can't be wrongly approved from a broken fetch, only wrongly held pending until the source recovers or the claimant/owner tries again).
   **Live-tested finding, since fixed:** a real `judge_claim` call was run against a live Bradbury deployment specifically to verify `gl.nondet.web.render` can reach these hosts from inside GenVM's sandbox — it can (real FlightAware content came back). But that run's `resultName` was `"DISAGREE"` (`DETERMINISTIC_VIOLATION` on 4/5 validator votes, one `TIMEOUT`): the bare `/live/flight/{ident}` page shows whichever flight is *currently* live, and independent validators fetching it at different moments got different content, failing `_facts_match`'s leader/validator agreement. Fixed by moving the fetch to `/history` (stable, completed past flights) and targeting the model at the policy's own bound `flight_date` specifically — see the "Fetching and authenticating..." section above. This fix has been redeployed but a second live judged claim confirming it resolves the disagreement has not yet completed as of this revision — do not treat flight settlement as fully live-verified until that follow-up run succeeds.
