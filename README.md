...
- **Flight Delay & Cancellation** — "Pay me 500 GEN if flight BA287 is delayed more than 3 hours or cancelled."
- **Weather & Drought** — "Pay me 2,000 GEN if Nakuru County receives less than 5mm of rain over any 15 consecutive days."
...
**Intelligent contract** (`contracts/LumenInsurance.py`) — policies, claims, and a shared payout pool, with GenLayer's Equivalence Principle used for claim adjudication. Judgment runs as two independent non-deterministic stages:

1. **Factual extraction** — objective facts only (flight delay minutes / cancellation, or dry-day count / rainfall) via an independent leader/validator agreement (`gl.vm.run_nondet_unsafe`).
2. **Intent judgment** — an LLM call (`gl.eq_principle.prompt_non_comparative`) decides whether those already-agreed facts satisfy the policy's plain-English intent, gated by a confidence threshold before any payout is possible.
...
tests/direct/                      gltest suite (62 tests)
...
- **Fail-closed everywhere** — malformed or non-JSON judgment output, an out-of-enum decision, or an inconsistent payout amount all degrade to a safe rejection, never a default approval.
