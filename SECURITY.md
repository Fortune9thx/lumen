...
5. **External data source availability is not guaranteed.** FlightAware and Open-Meteo are third-party services Lumen doesn't control; if either is unreachable or changes its page/response format, `_extract_claim_facts` degrades to `record_matches_*: False` (fail-closed — a claim can't be wrongly approved from a broken fetch, only wrongly held pending until the source recovers or the claimant/owner tries again).
   **Live reachability from GenVM validators is not yet verified.** `gltest`'s direct-mode `mock_web` proves the fetch/parse/binding *logic* is correct against a simulated response, but does not prove Bradbury's actual validator nodes can reach `flightaware.com` / `open-meteo.com` over the real internet from inside their GenVM sandbox at judgment time — that requires a real judged claim on a live deployment, which had not been run as of this revision. Until that's confirmed, treat the fetch-and-authenticate behavior described above as logically correct but operationally unverified in production; if the live fetch fails for a reason `gltest` can't simulate (DNS/sandbox network policy, a captcha/bot-block on either site, a response format change), the fail-closed behavior above still holds — claims would be safely stuck at `record_matches_*: False` rather than wrongly approved, but genuinely valid claims would then be unable to settle until this is confirmed and, if needed, fixed.
